...
export class AssetsViewerComponent implements OnInit, OnDestroy {
    ...
    constructor(private ui: UIService, private assetViewer: AssetViewerService, private data: DataService, private assetsApi: AssetsApi) {
        this.types = [];
        this.assets = [];
    }
    ...
    ngOnInit(): void {
        ...
        this.modalSubscription = this.ui.onModalChanged(function(modalName: string): void {
            if(modalName == null) {
                var filter: AssetsFilter = self.assetViewer.getFilter();
                self.loadAssets(filter);
            }
        });
    }
